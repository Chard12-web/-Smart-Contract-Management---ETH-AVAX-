...
export default function HomePage() {
  ...
  const handleTransaction = async (type, amount) => {
    if (atm) {
      try {
        const tx = type === "add" ? await atm.add(amount) : await atm.withdraw(amount);
        await tx.wait();
        fetchPoints();
        ...
        <div>
          ...
          <div>
            ...
            <button style={{
              ...
              borderBottomRightRadius: "30px",
              cursor: "pointer",
            }} onClick={() => handleTransaction("withdraw", document.getElementById("redeemAmount").value)}>Redeem Points</button>
          </div>
        </div>
      </div>
